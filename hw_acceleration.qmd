...
A popular class of neuromorphic algorithms are spiking neural networks (SNNs) [@maass1997networks]. Instead of modelling neurons as an activation function, spiking neural networks use neuron models imitating the temporal dynamics observed in the neurons of the brain. Many neuron models can be used, varing greatly in realism and complexity. On one end of the spectrum, there is realistic and complex models such as the Hodgin-Huxley model, which was derived by performing experiments on the axon of a giant squid (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1392413/). On the other end, there is the simpler models, such as the integrate-and-fire neuron. 

Arguably the most popular neuron model is the leaky integrate-and-fire model. Inputs to this neuron 'charge' the membrane potential according to the following equation: (ADD EQUATION), where the membrane leaks potential over time. Whenever the potential exceeds a threshold, the potential is reset and the neuron spikes. These discrete spikes are non-differentiable, and their occurance is highly dependent on the temporal dynamics. Therefore, training SNN can be a significant challenge. However, they have promising energy efficiency and responsive characteristics, when combined with neuromorphic sensors and hardware. TODO: clean text, write a sentence or two about neuromorphic benchmarking (NeuroBench :) ) and write about neuromorphic sensors.

(Unlike deep neural networks that communicate via continuous values, SNNs use discrete spikes more akin to biological neurons. This allows efficient event-based computation rather than constant processing. Additionally, SNNs take into account temporal characteristics of input data in addition to spatial characteristics. This better mimics biological neural networks, where timing of neuronal spikes plays an important role. However, training SNNs remains challenging due to the added temporal complexity. See following figure and video for reference -> Fun video but I dont really se its added value? A video showing the spiking communication might be more interesting)
